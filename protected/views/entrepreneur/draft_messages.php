...
<section class="admincontent">	
   ...
   <div class="composebg">
     ...
     <form name="fact_form" id="fact_form" action="<?php echo Yii::app()->params->base_path; ?>entrepreneur/deleteMessages" method="post" onSubmit="return redirect();">
    ...
    text-decoration: none;" />
			...
			<?php 
				...
				foreach($data['messages'] as $row) {
					...
					$userObj = new Users();
					$recData = $userObj->getUserById($row['receiver_id']);
			...
			?>  
              <tr <?php if($i%2!=0){ ?> class="oddrow bigrow" <?php } else { ?>class="evenrow bigrow" <?php } ?> >
               <td><input type="checkbox" name="check" value="<?php echo $row['message_id'] ; ?>" onchange="check_uncheck();"></td>
                  <td style="cursor:pointer;" onclick="showMessage('<?php echo $row['message_id']; ?>');"  class="msgrow"><font class="mailername"><a style="color:#10BEF8;" href="<?php echo Yii::app()->params->base_path ?>entrepreneur/showAdvisorProfile/userId/<?php echo $alogObj->encrypt($row['receiver_id'])  ; ?>" >
			  <?php echo $recData['firstName'].' '.$recData['lastName'] ; ?>
              </a></font><br /><?php echo $row['subject']; ?><br />
              <?php /*?><a href="<?php echo Yii::app()->params->base_path;?>entrepreneur/saveAsTrash/message_id/<?php echo $row['message_id'] ?>"><span>Delete</span></a><?php */?>
              </td>
              <td>
               <?php 
