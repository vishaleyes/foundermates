...
.result-description .results-text, .result-description .results-button {
display: table-cell;
...
.result-description .results-text STRONG {
height: 16px;
line-height: 16px;
font-size: 14px;
font-weight: bold;
...
function searchBy()
{
	...
	$j.ajax({
		type: 'POST',
		url: '<?php echo Yii::app()->params->base_path;?>site/searchAdvisors',
...
<div class="tab-content margintabcontent">
    <div class="result-description"><div class="results-text">
        <strong class="truncate">
        <?php $bool = false;?>
		<?php echo $data['pagination']->itemcount; ?> results 
        <strong style="color:#09F;">
		<?php if($ext['keyword']!="") { echo 'for '.trim($ext['keyword']); $bool = true; }?>
        <?php if($ext['city']!="") {if($bool == false) { echo 'for '; }  echo '   '.$ext['city'];$bool = true; } ?>
		<?php if($ext['country']!="") {if($bool == false) { echo 'for '; } echo '   '.$ext['country']; $bool = true;}?>
       <?php
        $industryObj = new Industry();
		$industryData = $industryObj->getIndustryForSearchById($ext['industry']);
		
		?>
        
		<?php if($ext['industy']!="") {if($bool == false) { echo 'for '; } echo '   '.$ext['industy']; $bool = true;} ?>
        <?php if($ext['area_of_expertise']!="") {if($bool == false) { echo 'for '; } echo '   '.$ext['area_of_expertise']; $bool = true;} ?>
        <?php if($ext['mentorship_experience']!="") {if($bool == false) { echo 'for '; } echo '   '.$ext['mentorship_experience'].' Years'; $bool = true; } ?>
        <?php if($ext['advisorType']!="") { if($ext['advisorType'] == 0) {if($bool == false) { echo 'for '; } echo ' Expert';$bool = true; } else {if($bool == false) { echo 'for '; } echo ' Mentor';$bool = true;} }?>  </strong>
        <?php  echo ' found'; ?> 
      </strong>
        <!--<em class="truncate">sorted by Klout Score</em>-->
    </div>
</div>
   ...
   $cnt = $data['pagination']->itemCount;
	...
	<tr class="rowborder">
        ...
        <td colspan="5" class="advisordetails" style=" width:57%;"><a style="color:#10BEF8;" href="<?php echo Yii::app()->params->base_path ?>site/showAdvisorProfile/userId/<?php echo $alogObj->encrypt($row['user_id'])  ; ?>" ><font class="advname"><?php echo $row['firstName']."&nbsp;".$row['lastName'] ; ?></font></a><br />
       <?php  //if(isset($row['tagline']) && $row['tagline'] != '') { echo $row['tagline']."<br />" ; }  ?> 
        ...
        <a href="<?php echo Yii::app()->params->base_path;?>site/entlogin"><span>Request Advice</span></a>
    ...
    <?php } } }else{  ?>
    <tr>
    	<td><b>Unfortunately, we don't have any advisors currently that match your criteria. Our system has already conveyed to us what you were looking for. Be rest assured that we will have relevant advisors on board soon.</b></td>
    </tr>
    <?php } ?>
